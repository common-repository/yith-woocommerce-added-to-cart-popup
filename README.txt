=== Plugin Name ===
Contributors: yithemes
Tags: added to cart, add to cart popup, cart, shop, shopping cart
Requires at least: 5.6
Tested up to: 5.8
Stable tag: 1.8.0
License: GPLv2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html

YITH WooCommerce Added to Cart Popup plugin allow display popup cart for add to cart action

== Description ==
YITH WooCommerce Added to Cart Popup gives you the freedom to continue browsing the shop or going directly to the cart.

Feel free to browse your favorite shop.

A simple plugin that offers an extremely useful option: with YITH WooCommerce Added To Cart Popup you will be free to add all the products you want to your cart without being forced to go to the cart page. Customize its style and its button, and offer the perfect bridge between the product page and the cart.

**Main features:**

* Open popup after add to cart in archive page
* Select to show buttons 'Continue Shopping' and 'View Cart'
* Customize button style

== Installation ==

1. Unzip the downloaded zip file.
2. Upload the plugin folder into the `wp-content/plugins/` directory of your WordPress site.
3. Activate `YITH WooCommerce Added to Cart Popup` from Plugins page.

== Screenshots ==

1. The plugin settings page.
2. The plugin popup

== Changelog ==

= 1.8.0 = Released on 05 November 2021

* New: support for WooCommerce 5.9
* Update: YITH plugin framework

= 1.7.1 = Released on 27 Sep, 2021

* Update: YITH plugin framework
* Fix: debug info feature removed for all logged in users

= 1.7.0 = Released on 10 Sep, 2021

* New: support for WooCommerce 5.7
* Update: YITH plugin framework

= 1.6.4 = Released on 10 Aug, 2021

* New: support for WooCommerce 5.6
* Update: YITH plugin framework

= 1.6.3 = Released on 06 Jul, 2021

* New: support for WordPress 5.8
* New: support for WooCommerce 5.5
* Update: YITH plugin framework

= 1.6.2 = Released on 01 Jun, 2021

* New: support for WooCommerce 5.4
* Update: YITH plugin framework

= 1.6.1 = Released on 03 May, 2021

* New: support for WooCommerce 5.3
* Update: YITH plugin framework

= 1.6.0 = Released on 09 April, 2021

* New: support for WooCommerce 5.2
* Update: YITH plugin framework

= 1.5.0 = Released on 02 March, 2021

* New: support for WordPress 5.7
* New: support for WooCommerce 5.1
* Update: YITH plugin framework

= 1.4.9 = Released on 02 February, 2021

* New: support for WooCommerce 5.0
* Update: YITH plugin framework

= 1.4.8 = Released on 11 January, 2021

* New: Support for WooCommerce 4.9
* Update: Plugin framework

= 1.4.7 = Released on 04 December, 2020

* New: Support for WooCommerce 4.8
* Update: Plugin framework

= 1.4.6 = Released on 02 November, 2020

* New: Support for WordPress 5.6
* New: Support for WooCommerce 4.7
* Update: Plugin framework

= 1.4.5 = Released on 12 October, 2020

* New: Support for WooCommerce 4.6
* Update: Plugin framework

= 1.4.4 = Released on 18 September, 2020

* New: Support for WooCommerce 4.5
* Update: Plugin framework

= 1.4.3 = Released on 19 August, 2020

* New: Support for WooCommerce 4.4
* New: Support for WordPress 5.5
* Update: Plugin framework

= 1.4.2 = Released on 21 May, 2020

* New: Support for WooCommerce 4.2
* Update: Plugin framework

= 1.4.1 = Released on 27 April, 2020

* New: Support for WooCommerce 4.1
* Update: Plugin framework

= 1.4.0 = Released on 07 March, 2020

* New: Support for WooCommerce 4.0
* New: Support for WordPress 5.4
* Update: Plugin framework

= 1.3.16 = Released on 20 December, 2019

* New: Support for WooCommerce 3.9
* New: Support for WordPress 5.3.2
* Update: Plugin framework

= 1.3.15 = Released on 29 November, 2019

* Update: Plugin framework

= 1.3.14 = Released on 5 November, 2019

* Update: Plugin framework

= 1.3.13 = Released on 30 October, 2019

* Update: Plugin framework

= 1.3.12 = Released on 25 October, 2019

* New: Support for WooCommerce 3.8
* New: Support for WordPress 5.3
* Update: Plugin framework

= 1.3.11 = Released on Aug 08, 2019

* New: Support for WooCommerce 3.7.0
* Update: Plugin Core

= 1.3.10 = Released on Jun 12, 2019

* Update: Plugin Core

= 1.3.9 = Released on Jun 11, 2019

* New: Support for WooCommerce 3.6.4
* New: Support for WordPress 5.2.1
* Update: Plugin Core
* Dev: New filter "yith_wacp_go_cart_url"
* Dev: New filter "yith_wacp_continue_shopping_url"

= 1.3.8 = Released on Apr 23, 2019

* Update: Plugin Core

= 1.3.7 = Released on Apr 03, 2019

* New: Support for WooCommerce 3.6.0 RC1
* New: Support for WordPress 5.1
* New: Option to show/hide product price in popup
* Update: Plugin Core

= 1.3.6 = Released on Feb 02, 2019

* New: Support for WooCommerce 3.5.4
* Update: Plugin Core

= 1.3.5 = Released on Dec 05, 2018

* New: Support for WooCommerce 3.5.2
* New: Support for WordPress 5.0.0
* Update: Plugin Core

= 1.3.4 = Released on Oct 24, 2018

* New: Support for WooCommerce 3.5.0
* Update: Plugin Core

= 1.3.3 = Released on Sep 28, 2018

* Update: Plugin Core

= 1.3.2 = Released on Sep 07, 2018

* New: Support for WooCommerce 3.4.5
* New: Support for WordPress 4.9.8
* Update: Plugin Core

= 1.3.1 = Released on May 30, 2018

* New: Support for WooCommerce 3.4.1
* New: Support for WordPress 4.9.6
* Update: Plugin Core

= 1.3.0 = Released on Feb 01, 2018

* New: Support for WooCommerce 3.3.0
* New: Support for WordPress 4.9.2
* New: Popup style
* New: Option to show/hide product image in popup
* Update: Plugin Core

= 1.2.0 = Released on Oct 17, 2017

* New: Support for WooCommerce 3.2.1
* New: Support for WordPress 4.8.2
* Update: Plugin Core

= 1.1.0 = Released on Mar 22, 2017

* New: Support to WooCommerce 3.0.0 RC1
* New: Support to WordPress 4.7.3
* Update: Plugin Core

= 1.0.7 = Released on Nov 28, 2016

* New: Support for WooCommerce 2.6.8
* Update: Plugin Core
* Fix: Load frontend class on plugin ajax call

= 1.0.6 = Released on Oct 24, 2016

* New: Support for WooCommerce 2.6.6
* Update: Change text domain from yith-wacp to yith-woocommerce-added-to-cart-popup
* Update: Language Files
* Update: Plugin Core
* Fix: Responsive popup style

= 1.0.5 = Released on Jun 10, 2016

* New: Support for WooCommerce 2.6 RC1
* Update: Plugin Core

= 1.0.4 = Released on Apr 14, 2016

* New: Added minimized js files. Plugin loads full files version if the constant "SCRIPT_DEBUG" is defined and is true
* New: Compatibility with WordPress 4.5
* Update: Plugin Core

= 1.0.3 = Released on Dec 28, 2015

* New: Support for WordPress 4.4
* New: Support for WooCommerce 2.5 BETA 3
* Update: Plugin Core
* Update: Language file

= 1.0.2 = Released on Aug 13, 2015

* New: Compatibility with WooCommerce 2.4
* Update: Plugin Core
* Update: Language file

= 1.0.1 = Released on Jul 27, 2015

* Fix: Minor bugs

= 1.0.0 = Released on Jun 23, 2015

* Initial release

== Suggestions ==

If you have suggestions about how to improve YITH WooCommerce Added to Cart Popup, you can [write us](mailto:plugins@yithemes.com "Your Inspiration Themes") so we can bundle them into YITH WooCommerce Added to Cart Popup.

== Translators ==

= Available Languages =
* English

If you can't find this plugin in your language and want to translate it, use the dedicated tool at [Translating WordPress](https://translate.wordpress.org/locale/it/default/wp-plugins/yith-woocommerce-added-to-cart-popup "Translating WordPress").
Or, if you have created your own language pack, or have an update for an existing one, you can send it as an email attachment from our [Contacts page](http://yithemes.com/contact/ "Your Inspiration Themes") and we will bundle into YITH WooCommerce Added to Cart Popup.

== Upgrade notice ==

= 1.7.1 = Released on 27 Sep, 2021

* Update: YITH plugin framework
* Fix: debug info feature removed for all logged in users
